...
# run our differential equation solver
res = ode(func=lotvmodK, y=currpop, times=days, parms=pars_stab, min_prey_pop = 0)
res_smallstart = as.data.frame(res) %>%
  gather(key = "species", value = "pop", -time)

ggplot(res_smallstart, aes(time, pop, col=species)) +
  geom_line()



stable_pop = c(prey = preyi, pred = predi)

res = ode(func=lotvmodK, y=stable_pop, times=days, parms=pars_stab, min_prey_pop = 0)
stable_start = as.data.frame(res) %>%
  gather(key = "species", value = "pop", -time)

ggplot(stable_start, aes(time, pop, col=species)) +
  geom_line()
```


```{r}
pars = data.frame(rprey=0.1, alpha=0.6, eff=0.8,pmort=0.4, K=200, beta = 0.05)
...
We're defining stability as when populations do not change. 
